 ...
 * 가상 팀 프로젝트 — 페르소나 2명(이용 유저 + 예비 창업주) 관점.
 * 각 섹션 전체 캡처(tonefit-case/*)를 브라우저 창(내부 스크롤)에 담아 소개한다.
 */

const IMG = "/assets/tonefit"; // 01 히어로 목업(기존)
const CASE = "/assets/tonefit-case"; // 전체 섹션 캡처
...
const UX_POINTS = [
  ...
  {
    desc: "한 번 찍으면 다시 올 이유가 없다는 것이 셀프 스튜디오의 약점입니다. IP 콜라보 프레임을 매달 교체하는 구조로 만들어, 사용자에게는 재방문 이유를, 점주에게는 별도 제작 없이 채워지는 콘텐츠를 확보했습니다.",
    img: "frame-pc",
    alt: "콜라보 프레임",
  },
  {
    desc: "촬영은 세 단계입니다. STEP 01 카메라 앞에 서면 30초 안에 피부 톤·명도·채도를 분석하고, STEP 02 결과에 맞는 프레임·조명·배경을 자동 세팅, STEP 03 촬영 후 인화·결제까지. 직원 없이 화면 안내만 따라가면 되도록 이용 흐름을 짰습니다.",
    img: "kiosk-pc",
    alt: "키오스크 이용 STEP",
  },
];
...
function Point({ desc, img, alt }: { desc: string; img: string; alt: string }) {
  return (
    <div className={styles.point}>
      ...
      <Reveal delay={0.08} className={styles.pWindow}>
        ...
        <div className={styles.winScroll}>
          <CaseImage src={`${CASE}/${img}.png`} alt={alt} />
        </div>
      </Reveal>
    </div>
  );
      ...
      <div className={styles.page}>
        ...
        <section className={`${styles.section} ${styles.alt}`}>
          <Reveal className={styles.head}>
            ...
            <p className={styles.headLead}>
              톤핏은 찍으러 오는 유저만큼, 매장을 여는 점주도 고객으로 봤습니다.
              앞선 유저 경험을 근거로 — 인건비 없는 무인 운영, 3개월 누적 15만
              건의 성장, 상권별 매출, 가맹 경쟁력, 계약 프로세스까지 — 한 흐름으로
              정리해, 관심을 실제 창업 문의로 잇도록 구성했습니다.
            </p>
          </Reveal>
          ...
          <Reveal className={styles.startupWindow}>
            ...
            <div className={styles.winScroll}>
              <CaseImage
                ...
                alt="TONE:FIT 창업 섹션 — 무인운영·성장·매출·가맹·계약"
              />
            </div>
          </Reveal>
        </section>
